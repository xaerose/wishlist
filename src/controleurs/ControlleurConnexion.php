...
class ControlleurConnexion
{
    ...
    {
        $vue = new \mywishlist\vues\VueConnexion(array($requete));
        $html = $vue->render(1);
                    ...
                    if (strlen($email) <= 100) { // On verifie que la longueur du mail <= 100
                        if (filter_var($email, FILTER_VALIDATE_EMAIL)) { // Si l'email est de la bonne forme
                            if ($password === $password_retype) { // si les deux mdp saisis sont bon
                                ...
                                return $rs->withRedirect('/index.php/inscription?res=Inscription Reussi');
                            } else {
                                return $rs->withRedirect('/index.php/inscription?res=Mdp Pas Identique');
                            }
                        } else {
                            return $rs->withRedirect('/index.php/inscription?res=Format Mail Incorrect');
                        }
                    } else {
                        return $rs->withRedirect('/index.php/inscription?res=Email trop long');
                    }
                } else {
                    return $rs->withRedirect('/index.php/inscription?res=Pseudo trop long');
                }
            } else {
                return $rs->withRedirect('/index.php/inscription?res=Vous etes deja inscrit!');
            }
        }
        return $rs;
    }
